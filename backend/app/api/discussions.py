...
def list_threads(
    ...
    # Base query parameters for PostgREST
    ...
    # Apply query filtering if keyword search is requested
    if q:
        params["title"] = f"ilike.*{q}*"
        
    threads = supabase_get("threads", params)
    
    # Flatten author email and category name for ease of frontend consumption
    formatted_threads = []
    for t in threads:
...
def create_thread(payload: ThreadCreate, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        # 2. Generate Thread Embedding (for similarity searches)
        ...
        # 3. Generate automated AI RAG response (Amazon customer service flow)
        try:
            ...
            if rag_res.get("ai_answer"):
                ...
                ai_text = f"Hi! I searched our official guidelines and found this related topic:\n\n**{top_faq['question']}**\n{top_faq['answer']}\n\n*If this does not answer your question, click the 'Escalate to Staff' button in the sidebar to summon a human mentor.*"
            
            if ai_text:
                # Insert automated reply from VINS support bot
                supabase_post("replies", {
                    "thread_id": thread_id,
                    "user_id": None, # None represents VINS Support Bot in client
                    "content": ai_text,
                    "upvotes": 0,
                    "is_accepted": False,
...
def reply_to_thread(id: str, payload: ReplyCreate, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        # If user is a mentor or admin, reward them reputation points
        if is_mentor:
    ...
    try:
        ...
        if existing:
            ...
            if old_vote["vote_value"] == payload.vote_value:
                # Same vote, do nothing or retract
        ...
        # 2. Update upvote count in target item
        ...
        if target_item:
            ...
            # 3. Update reputation points of author
            ...
            if author_id:
                ...
                if author:
                    ...
                    # Update author role if they hit Trusted Contributor reputation threshold
        ...
        return {"status": "success", "new_upvotes": new_upvotes if 'new_upvotes' in locals() else 0}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def accept_reply(id: str, reply_id: str = Query(...), user: dict = Depends(require_auth)):
    ...
    # Only thread creator, mentors, or admins can accept answers
    ...
    try:
        # Reset all accepted flags for this thread
        ...
        # Mark target reply as accepted
        supabase_patch("replies", {"id": f"eq.{reply_id}"}, {"is_accepted": True})
        
        # Reward reputation to the helpful user (+15 points)
        target_reply = supabase_get("replies", {"id": f"eq.{reply_id}"})[0]
        helper_id = target_reply["user_id"]
        if helper_id:
