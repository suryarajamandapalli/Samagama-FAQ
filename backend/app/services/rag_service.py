...
def query_rag(question: str, category_slug: str = None) -> dict:
    ...
    try:
        ...
        faq_matches = supabase_rpc("match_faqs", faq_params)
        thread_matches = supabase_rpc("match_threads", thread_params)
    except Exception as e:
    ...
    # Calculate similarities and determine confidence
    max_faq_similarity = max([item["similarity"] for item in faq_matches]) if faq_matches else 0.0
    max_thread_similarity = max([item["similarity"] for item in thread_matches]) if thread_matches else 0.0
    
    best_similarity = max(max_faq_similarity, max_thread_similarity)
    
    if best_similarity >= 0.70: # Lowered threshold slightly for difflib/ratio overlap tolerance
        confidence = "high"
    ...
    ai_answer = None
    if confidence == "high":
        # Generate RAG response using top matches
        context_parts = []
...
"""
    ...
    # Extract first answer from context as fallback if LLM fails/expires
